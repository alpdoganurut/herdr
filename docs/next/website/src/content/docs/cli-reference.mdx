...
herdr tab color <tab_id> <red|orange|yellow|green|cyan|blue|purple|none>
herdr tab remind <tab_id> <on|off>
...
`tab color` tags a tab with a named theme color (`tab.set_color`) and prints the tab; `none` clears the tag. The color is stored with the tab, survives server restarts, and follows the tab when it moves to another space. The `tabs` sidebar layout draws the tab's name in the color, and the `spaces` layout's tab bar tints unfocused tab names with it.

`tab remind` marks a tab for idle reminders (`tab.set_remind`) and prints the tab; `off` removes the mark. While a marked tab's agent sits finished and unseen, or blocked, the Herdr client reminds you every `ui.idle_reminder_minutes` until you focus the tab. The mark is stored with the tab, survives server restarts, and follows the tab when it moves to another space.
...
herdr agent suspend <target>
herdr agent activate <target>
herdr agent restart <target>
herdr agent explain <target> [--json|--verbose]
herdr agent explain --file PATH --agent LABEL [--json|--verbose]
herdr agent transcripts [--json]
```
...
`idle` and `done` both mean ready for input. The CLI/API uses the server's seen state: `done` is idle but not yet marked seen, explicit `pane focus` / `agent focus` commands mark the target seen, and reads do not. Each TUI client tracks viewed completions independently, so its Done badge can differ from the CLI or another client. `blocked` means Herdr recognized an approval or question UI. `unknown` means an agent is present but Herdr cannot classify it confidently, not that its work succeeded. `suspended` means the agent was parked with `agent suspend`; its process is exiting or has exited, and the pane accepts no agent input until `agent activate` relaunches it.

`agent suspend` asks a running agent to exit from its own prompt while the pane keeps the stored native session reference and the agent name. It requires an official session reference, a native resume plan, and a known graceful exit command (currently Claude Code); otherwise it returns `agent_not_suspendable`. A `blocked` agent is refused with `agent_blocked` and a `working` one with `agent_working` (wait for it to go idle); nothing is sent to the agent then. If the process has not exited after a short grace period, Herdr terminates and then kills the foreground job, never the pane shell. The pane reports `suspended` immediately, stays listed under its name, and survives server restarts as suspended without being relaunched. `agent activate` relaunches the suspended agent in the same pane with its native resume command; the pane must be back at an available shell prompt or the command returns `pane_not_available`. Both print the pane id on success.

`agent restart` exits an idle agent and relaunches it in place with its native resume command, in one command: it suspends the agent like `agent suspend`, then Herdr runs the resume command on its own once the exit is observed and the shell prompt is back, so no `agent activate` is needed. A `working` agent is refused with `agent_working`, a `blocked` one with `agent_blocked`, and a `suspended` one with `agent_suspended` (activate it instead). If the relaunch cannot happen (the agent had to be killed, or the shell prompt does not return within 30 seconds) the pane stays suspended. Prints the pane id on success; follow with `agent wait <target>`.

`agent transcripts` lists the native transcript backups Herdr keeps for open and suspended agent panes under the session directory (`agent-transcripts/`): agent, session id, size, backup time, and whether the agent's own transcript file is currently `present` or `missing`. It reads the store directly, so it works without a running server and is not forwarded to a saved machine. Add `--json` for machine-readable output. See [Session state](/docs/session-state/) for how backups are refreshed and put back before a resume.
