...
On macOS and Linux the install also adds `SubagentStart` and `SubagentStop` entries that run the same script with the `subagent` action; it reports each subagent starting and stopping (`pane.report_subagent`), and the `tabs` sidebar layout shows `◎` as a working tab's status icon while its agent has subagents running. A server without that method ignores the reports.
