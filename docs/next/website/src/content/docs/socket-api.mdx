...
| Tab | `tab.create`, `tab.list`, `tab.get`, `tab.focus`, `tab.rename`, `tab.set_color`, `tab.set_remind`, `tab.move`, `tab.close` |
| Pane | `pane.split`, `pane.swap`, `pane.move`, `pane.zoom`, `pane.layout`, `pane.process_info`, `pane.neighbor`, `pane.edges`, `pane.focus_direction`, `pane.resize`, `pane.list`, `pane.current`, `pane.get`, `pane.rename`, `pane.send_text`, `pane.send_keys`, `pane.send_input`, `pane.read`, `pane.graphics.info`, `pane.graphics.set`, `pane.graphics.clear`, `pane.graphics.stream`, `pane.report_agent`, `pane.report_agent_session`, `pane.report_subagent`, `pane.report_metadata`, `pane.clear_agent_authority`, `pane.release_agent`, `pane.close`, `pane.wait_for_output` |
| Popup | `popup.close` |
| Layout | `layout.export`, `layout.apply`, `layout.set_split_ratio` |
| Agent | `agent.list`, `agent.get`, `agent.read`, `agent.explain`, `agent.send_keys`, `agent.prompt`, `agent.wait`, `agent.rename`, `agent.focus`, `agent.start`, `agent.suspend`, `agent.activate`, `agent.restart`, `agent.transcripts`, `agent.view.set`, `agent.view.clear` |
...
`agent.suspend` asks a running agent to exit from its own prompt while the pane keeps the stored native session reference and the agent name; the pane reports `suspended` from that moment. It requires an official session reference, a native resume plan, and a known graceful exit command (currently Claude Code), otherwise it returns `agent_not_suspendable`. If the process has not exited after a short grace period, Herdr terminates the agent processes in the foreground job (`SIGTERM`, then `SIGKILL` on Unix; `TerminateProcess` on Windows), never the pane shell. While the agent is suspended, `agent.prompt`, `agent.send_keys`, and `agent.rename` return `agent_suspended`; a `blocked` agent cannot be suspended (`agent_blocked`), a `working` agent is refused with `agent_working` until it goes idle, and no input is sent in either case; an agent that is the pane's own process (an imported launch command) cannot be suspended (`agent_not_suspendable`). `agent.activate` relaunches the suspended agent in the same pane with the native resume command; the pane must be back at an available shell prompt and the exit must have been observed, or it returns `pane_not_available`. Both accept `target` as a pane id or agent name and return `pane_id`. Suspended panes survive server restarts as suspended and are not relaunched automatically.

`agent.restart` exits a live agent and relaunches it in the same pane with its native resume command in one request. It refuses a `working` agent with `agent_working` (wait for it to go idle), a `blocked` agent with `agent_blocked`, and a `suspended` agent with `agent_suspended` (use `agent.activate` instead); otherwise it applies every `agent.suspend` precondition (`agent_not_suspendable` and the rest). On success it performs the suspend, so the pane reports `suspended` in between exactly as with a manual suspend, and returns `pane_id` right away. Herdr then relaunches the agent automatically, with no second request, as soon as the exit has been observed and the pane is back at an available shell prompt. If that never happens (the exit needed `SIGKILL`, or the exit or the shell prompt does not arrive within 30 seconds) the pane simply stays suspended and the server logs a warning; `agent.activate` still works. A restart in flight does not survive a server restart: the pane comes back suspended.

`agent.transcripts` reports the native transcript backup store (see [Session state](/docs/session-state/)): `store_dir`, whether passes run (`enabled`, the `session.backup_agent_transcripts` flag together with session persistence), `sessions` with a backup and how many of those are `native_missing`, `transcript_bytes` recorded in their metadata, `disk_bytes` for everything below the store, the `last_pass` finished since the server started (`finished_unix`, `duration_ms`, and `updated`/`unchanged`/`skipped`/`failed` counts) when there is one, and `next_pass_in_ms` while passes are scheduled. It reads the store on request and never changes it.

`tab.set_color` tags a tab with a named color: `{ "tab_id": "...", "color": "red" }`, where `color` is one of `red`, `orange`, `yellow`, `green`, `cyan`, `blue`, `purple`, or `null` (or omitted) to clear it. It returns the tab like `tab.rename`; an unknown tab returns `tab_not_found` and an unknown color name `invalid_tab_color`. Tab records (`tab.get`, `tab.list`, tab responses and events) carry the tag as an optional `color` field that is absent when the tab has none; clients should treat a color name they do not know as no color. The color is persisted with the session and follows the tab when `pane.move` moves a tab's only pane into a new tab or workspace (a pane split out of a larger tab starts uncolored). No event is emitted for a color change.

`tab.set_remind` marks a tab for idle reminders: `{ "tab_id": "...", "remind": true }`, or `false` to remove the mark (`remind` is required). It returns the tab like `tab.rename`; an unknown tab returns `tab_not_found`. Tab records carry the mark as an optional `remind: true` that is absent when the tab is unmarked. The mark is persisted with the session and follows the tab when `pane.move` moves a tab's only pane into a new tab or workspace. Clients time the reminders themselves (see `ui.idle_reminder_minutes`); no event is emitted for a mark change.
...
Effective status values are `idle`, `working`, `blocked`, `done`, `unknown`, and
`suspended`; `done` means idle and not yet seen.
...
`pane.report_subagent` reports a Claude Code subagent starting or stopping under the pane's agent (the Claude integration's `SubagentStart` and `SubagentStop` hooks send it): `{ "pane_id": "...", "agent": "claude", "event": "start", "subagent_id": "..." }`, with `event` `start` or `stop`. A report for an agent the pane no longer runs is ignored; an empty or overlong `subagent_id` returns `invalid_subagent_id`. Herdr keeps at most 64 running subagents per pane, never persists them, and forgets them when the agent stops working, is suspended, released or replaced. While the agent works, `agent.get`, `agent.list` and agent events carry the count as an optional `subagents` field, absent when zero.
