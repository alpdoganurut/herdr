...
`agent.suspend` asks a running agent to exit from its own prompt while the pane keeps the stored native session reference and the agent name; the pane reports `suspended` from that moment. It requires an official session reference, a native resume plan, and a known graceful exit command (currently Claude Code), otherwise it returns `agent_not_suspendable`. If the process has not exited after a short grace period, Herdr sends `SIGTERM` and then `SIGKILL` to the foreground job, never to the pane shell. `agent.activate` relaunches the suspended agent in the same pane with the native resume command; the pane must be back at an available shell prompt or it returns `pane_not_available`. Both accept `target` as a pane id or agent name and return `pane_id`. Suspended panes survive server restarts as suspended and are not relaunched automatically.
