...
A pane hosting a suspended agent comes back as a shell that still reports `suspended`, keeps the agent name and native session reference, and is never relaunched automatically; use `agent activate`. Because no process survives a restart, the restored record treats the exit as already observed. Only a live handoff carries an exit still in progress across the replacement, resuming the wait on the new server.
...
Some agents delete their own conversation transcripts after a period of inactivity, and a later native resume then finds no conversation. Herdr keeps a copy of the transcript behind every open or suspended agent pane under the session directory at `agent-transcripts/<agent>/<session-id>/` (`transcript.jsonl`, a `side/` directory for tool results and subagent transcripts, and `meta.json` with the original path, pane directory, and backup time). Copies are refreshed every five minutes, when an agent is suspended, and on shutdown; only sources that changed are copied, and Herdr never deletes a backup. When the agent's own transcript shrinks (for example after it was truncated or recreated), the previous copy is kept as `transcript.prev.jsonl` next to the new one. Prune a backup you no longer need by deleting its `agent-transcripts/<agent>/<session-id>/` directory. Before a native resume, whether after a restart or through `agent activate`, Herdr puts the backup back in place if the agent's own file is missing, and never overwrites one that exists. Currently Claude Code transcripts are backed up. Disable new backups with `backup_agent_transcripts = false` under `[session]`; existing backups are still used before a resume. `herdr agent transcripts` lists the store; the settings overlay's `backups` tab shows the same totals, the last pass, and the next scheduled one (`agent.transcripts` over the socket API).
