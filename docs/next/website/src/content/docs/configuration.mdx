...
`ui.sidebar_layout = "tabs"` replaces the Space and Agent panels with one row per tab across every space, in tab order. Each row shows the tab's agent status; click focuses the tab and right-click opens the tab menu. The horizontal tab bar is dropped, and `next_tab` / `previous_tab` cycle the whole list across spaces. The layout keeps one pane per tab, so the pane-level actions are inert: split, swap, zoom, resize, pane focus and cycling, last pane, close pane and rename pane. A tab that already holds several panes (created through the API, a layout, or before switching layouts) keeps only mouse focus between them. A right-click on the pane opens the tab menu. A suspended agent pane shows a card instead of its shell and ignores input until `keys.toggle_agent_suspend` activates it again. Each row ends with an agent glyph from `ui.tab_agent_glyphs`; on the focused row that glyph takes the agent's color from `ui.tab_agent_glyph_colors` (default `claude = "#D97757"`, `codex = "#3B82F6"`; hex or named colors, and agents without a color stay monochrome). The tab menu's "Restart agent" item (and `keys.restart_agent`, unset by default, in either layout) exits a live, idle agent and relaunches it in place with its native resume command; a refusal such as a working agent shows a short notice. A tab can carry a color tag (red, orange, yellow, green, cyan, blue or purple, drawn with the theme's red, peach, yellow, green, teal, blue and mauve): the row draws the tab's name in that color, focused or not, while the status icon, glyph and row highlight stay as they are. Pick it from the row of swatches at the bottom of the tab menu (`∅` and four colors, red, yellow, green and purple, drawn in fixed true colors so they stay distinct on any theme; the current one is bracketed): click one, or move to the row with the up and down arrows, choose with the left and right arrows or `h`/`l`, and press Enter; `∅` clears it, cycle the focused tab through those four with `keys.cycle_tab_color` (unset by default), or use `herdr tab color`. The color belongs to the tab, so it persists across restarts and follows the tab into another group; the `"spaces"` layout's tab bar tints unfocused tab names with it. The default `"spaces"` keeps the two-panel layout.

In this layout spaces act as tab groups. The first space is the ungrouped bucket and lists its tabs without a header; every other space renders as a group with a header row (fold marker, name, member count, rolled-up status) above its tabs. Click a header to fold or unfold that group, use the `⏶` button at the top of the list to fold every group (it turns into `⏷` once all are folded, and then expands them; `keys.toggle_groups_folded` does the same from the keyboard), and `+` (or `keys.move_tab_to_group`) to move the focused tab to a group by name, creating the group when the name is new. A group holding the focused tab is always drawn open. Drag a header to reorder groups, drag a tab row to reorder it inside its group or drop it on another group to move it there (it lands at the end of that group, and the drop line shows that). Group moves apply to single-pane tabs only, and the ungrouped list always keeps at least one tab; a move that would break either rule is refused with a notice. The group holding the focused tab cannot be folded. Right-click a header for Rename group, Ungroup (members return to the bucket) and Close group; a group whose last tab closes disappears. Fold state is a client preference, so it survives restarts.
...
- `state_text` — `idle`, `working`, `blocked`, `done`, `unknown`, or `suspended`, including a reported display label when present.
...
With `delivery = "herdr"`, `ui.toast.herdr.sticky = true` keeps toasts on screen until you handle them instead of expiring them. Cards stack from the configured corner with the newest nearest it; beyond `ui.toast.herdr.max_stack` cards (default 6, 1 through 20), or half of the frame height, older cards fold into one "+N more" line. Left-click a card to focus its pane, right-click to dismiss it, or press `keys.open_notification_target` for the newest card. A card also clears when its tab becomes focused, its pane closes, a needs-input agent is no longer blocked, a finished agent starts working again, or a newer notification for the same pane replaces it.

Mark a tab for idle reminders with the tab menu's "Remind me" item (in either sidebar layout; "Stop reminding" removes the mark), `keys.toggle_tab_remind` (unset by default) for the focused tab, or `herdr tab remind <tab_id> on`. The `tabs` sidebar layout shows a `◷` before a marked row's agent glyph. While a marked tab's agent sits finished and unseen, or blocked, Herdr reminds you after `ui.idle_reminder_minutes` (default 10) and again every as many minutes until you focus the tab, the agent works again, or you remove the mark; an agent you have already seen never reminds. The reminder ("planner finished 20 min ago", "planner still waiting") follows `delivery` like any other notification, replaces that tab's previous card instead of stacking a new one, and plays the done or request sound unless sounds are off. `ui.idle_reminder_minutes = 0` turns reminders off (at most 240); the settings overlay's **reminders** tab offers off, 5, 10, 15, 30 and 60 minutes. The mark belongs to the tab, so it persists across restarts and follows the tab into another group; the reminder clock is kept by each client and starts over when the client restarts.
