...
`ui.sidebar_layout = "tabs"` replaces the Space and Agent panels with one row per tab across every space, in tab order. Each row shows the tab's agent status; click focuses the tab and right-click opens the tab menu. The default `"spaces"` keeps the two-panel layout.
